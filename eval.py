...
def get_sample_metrics(batch_dict, score_sample_dict):
    ...
    for key, val in score_sample_dict.items():
        out_dict[key] = dict()
        assert labels.shape == val.shape
        out_dict[key] = get_metrics(val, labels)
    return out_dict
...
def eval_ano_algo(ano_algo, model_path, loader_dict, scoring=['sample', 'pixel'],vis=True, name_exp='fast'):
    ...
    for dataset, loader in loader_dict.items():
        ...
        loader_pixels = dict() 
        if 'pixel' in scoring:
            for name, kw_px in kwargs_eval.items():
                print("Compute Scores over Dataset")
                batch_dict, score_sample_dict, score_pixel_dict = evaluate_ano_loader(ano_algo=ano_algo, dataloader=loader, kwargs_pix=kw_px, scoring=['sample' ,'pixel'])
...
def get_data_loaders(name_exp):
    ...
    for key in keys:
        exp_loader_dict[key] = get_brain_dataset(**arg_data['common_args'], **eval_loader_args[key])
