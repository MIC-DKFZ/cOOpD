...
from datamodules.lung import get_lung_dataset, init_arg_dicts, get_lung_dataset_eval
from config.datasets.lung import get_lung_args
import subprocess

## Readout config values
lung_base_args = get_lung_args(mode='train')
## Args for Datasets used for the contrastive Training (& the generative models)!
datasets_common_args = lung_base_args['common_args']
datasets_train_args = lung_base_args['trainset_args']
datasets_val_args = lung_base_args['valset_args']

## Args for Validation Datasets containing anomalies which are labelled!
datasets_val_ano_args = lung_base_args['valanoset_args']
datasets_test_args = lung_base_args['testset_args']


def get_lung_datasets(common_args=None, trainset_args=None, valset_args=None, valanoset_args=None, testset_args=None):
    ...
    train_loader, val_loader = get_lung_dataset(**d_common_args, **d_train_args)
    val_ano_loader = get_lung_dataset(**d_common_args, **d_val_ano_args)
    test_loader = get_lung_dataset(**d_common_args, **d_test_args)
...
class LungDataModule(pl.LightningDataModule):
    ...
    def __init__(self, batch_size: int = 64, double_headed: bool = False, patch_size: int = (50, 50, 50),
                 input: str = 'insp', overlap: str = '20', kfold: int = 1, max_patches: int = None,
                 base_train='default', base_dir: list = [], mode='train', step='pretext', realworld_dataset=False,
                 *args, **kwargs):
        super().__init__()
        ...
        self.args = {'batch_size': batch_size, 'double_headed': double_headed,
                     ...
                     'realworld_dataset': realworld_dataset, 'input': input, 'overlap': overlap, 'kfold': kfold,
    ...
    def train_dataloader(self):
        return get_lung_dataset(**self.common_args, **self.train_args)

    def val_dataloader(self):
        return get_lung_dataset(**self.common_args, **self.val_args)

    def test_dataloader(self):
        return get_lung_dataset_eval(**self.common_args, **self.test_args)

    def get_ano_loader(self):
        raise NotImplementedError

    def get_lung_datasets(self):
        return get_lung_datasets(self.common_args, self.train_args, self.val_args,
    ...
    def add_data_specific_args(parent_parser):
        ...
        parser.add_argument("--resume", type=str,
                            default=None)  # '/home/silvia/Documents/CRADL/logs_cradl/copdgene/pretext/lung/simclr-resnet18/default/17030592/checkpoints/epoch=16-step=133585.ckpt')

        # Training specific arguments
        # parser.add_argument("--num_workers", default=8, type=int) #8 #12
        parser.add_argument("--dataset", default='lung', type=str)  # choices=['lung'],type=str)
